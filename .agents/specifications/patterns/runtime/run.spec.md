...
- If a rule name is specified, the `run` pattern MUST look up that name from the
  active module's exports.
...
- If the target rule is resolved, the `run` pattern MUST evaluate that rule in
  the current scope.
...
- On success, the `run` pattern MUST report the resolved rule's matched value as
  its output value.
