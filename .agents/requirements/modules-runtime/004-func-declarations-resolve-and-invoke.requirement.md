...
title: Func declarations resolve, export, import, and invoke with pattern args
...
- Func argument lists are matched with the declared parameter pattern.
...
- A module that declares and exports a func MUST expose that func on
  ...
  projection in the same module when arguments match the parameter pattern.
...
- Invoking a func with arguments that fail the parameter pattern MUST fail the
  invocation (not silently coerce).
