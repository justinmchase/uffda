...
- A rule with one or more decorators produces a successful `MatchOk`, either
  through fresh evaluation or a memoized cache hit.

Expected behavior:

- `MatchOk.origin.rule.metadata` MUST reflect the decorated rule's merged
...
- Consumers (for example semantic-highlighting tooling) MUST be able to inspect
  a rule's decorator list and metadata via `origin.rule` without re-invoking any
  decorator.
