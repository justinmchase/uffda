...
Deno.test("req:runtime-core-005 - Failed and zero-width outcomes preserve caller-visible input position", async (t) => {
  await t.step("failed match preserves caller-visible position", () => {
  ...
  await t.step("zero-width not preserves caller-visible position", () => {
    const scope = Scope.From("a", { kind: InputNormalizationMode.Iterable });
  ...
  await t.step("input-consuming success reports updated position", () => {
    const scope = Scope.From("a", { kind: InputNormalizationMode.Iterable });
