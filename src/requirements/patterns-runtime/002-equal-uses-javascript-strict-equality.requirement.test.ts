...
Deno.test("req:equal-002 - Equal compares the current item using JavaScript strict equality", async (t) => {
  await t.step("strict equality matches identical primitive value", () => {
  ...
  await t.step("strict equality rejects type-coercive near match", () => {
    const scope = Scope.From(["7"], { kind: InputNormalizationMode.Iterable });
    const pattern: Pattern = { kind: PatternKind.Equal, value: 7 };
  ...
  await t.step("strict equality on objects requires same reference", () => {
    const shared = { x: 1 };
    ...
    const okMatch = match(pattern, scopeOk);
    const failMatch = match(pattern, scopeFail);
