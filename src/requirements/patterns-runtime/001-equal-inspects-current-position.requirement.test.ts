...
Deno.test("req:equal-001 - Equal inspects the current input position and fails at end-of-input", async (t) => {
  await t.step("equal inspects and matches at the current position", () => {
    ...
    const m = match(pattern, atB);
    assertEquals(m.kind, MatchKind.Ok);
  });

  await t.step("equal fails at end-of-input without changing position", () => {
