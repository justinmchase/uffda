...
export class Resolver {
  ...
  private async resolveImport(normalizedModuleUrl: string): Promise<IModule> {
    if (this.modules.has(normalizedModuleUrl)) {
      ...
      const moduleDeclaration: IModuleDeclaration = await resolver.resolveModule(normalizedModuleUrl)
      return await this.resolveModule(normalizedModuleUrl, moduleDeclaration);
    }
  }
  
  private async resolveModule(normalizedModuleUrl: string, moduleDeclaration: IModuleDeclaration): Promise<IModule> {
    if (this.modules.has(normalizedModuleUrl)) {
      return this.modules.get(normalizedModuleUrl)!
    } else {
      ...
      for (const i of moduleDeclaration.imports) {
        const importModuleUrl = Resolver.normalizeModulePath(i.moduleUrl, normalizedModuleUrl);
  ...
  public async load(moduleUrl: string, moduleDeclaration?: IModuleDeclaration): Promise<IModule> {
    const normalizedModuleUrl = Resolver.normalizeModulePath(moduleUrl, this.moduleUrl);
  ...
  public static normalizeModulePath(modulePath: string, parentPath: string) {
    const { origin, pathname } = (() => {
      if (modulePath.match(/^file:\/\//)) {
        ...
        return {
          origin: 'file://',
          pathname: path.resolve(path.dirname(path.fromFileUrl(parentPath)), modulePath)
        }
